...
class Sortable {
	...
	orderelements() {
		...
		let arrayRectHeight = activeElements.reduce((acc, el, id) => {
			...
			let rectHeight = (id - columns >= 0) ? (columnsHeight[id % columns] + (margin * Math.floor(id /
				columns))) : 0
		...
		let parentHeight = Math.max(...columnsMaxHeight) + (margin * (Math.floor(activeElements.length / columns) -
			1))
